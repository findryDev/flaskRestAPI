...
howMany = 800
overHeatTemp = 75
cache.set("overHeatTemp", datetime.datetime.now() - datetime.timedelta(minutes=5))
...
class TemperaturesView(Resource, Apicheck):
    ...
    def post(self):
        try:
            ...
            checkDict = Apicheck.checkingApiKey()
            overHeatTemp = cache.get("my_value")
            if checkDict['check']:
                ...
                if (data['temperature'] > overHeatTemp and overheatTime + datetime.timedelta(minutes=5) > datetime.datetime.now()) :
                    ...
                    overheatTime = datetime.datetime.now()
