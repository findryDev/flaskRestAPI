...
howMany = 800
overHeatTemp = 75
cache.set("overheatTime", datetime.datetime.now() - datetime.timedelta(minutes=5))
    ...
    def post(self):
        try:
            ...
            if checkDict['check']:
                ...
                print(overheatTime, data['temperature'], (overheatTime + datetime.timedelta(minutes=5)) < datetime.datetime.now())
                if (float(data['temperature']) > overHeatTemp and (overheatTime + datetime.timedelta(minutes=5)) > datetime.datetime.now()):
                    iftttOverheat(float(data['temperature']))
                    cache.set("overheatTime", datetime.datetime.now())
                    print('overheat')
