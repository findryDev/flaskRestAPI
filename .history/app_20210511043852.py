...
app = Flask(__name__)
cache.init_app(app=app, config={"CACHE_TYPE": "filesystem",'CACHE_DIR': os.path('/tmp')})
    ...
    def post(self):
        try:
            ...
            if checkDict['check']:
                ...
                if (data['temperature'] > overHeatTemp and overheatTime + datetime.timedelta(minutes=5) > datetime.datetime.now()):
                    iftttOverheat(data['temperature'])
